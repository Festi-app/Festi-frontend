...
import { MobileWaitingRegister, MobileWaitingStatus } from './screens/mobile-waiting'
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="rounded-lg px-2.5 py-1 text-xs font-semibold cursor-pointer border"
        style={{ borderColor: 'var(--border-color)', background: 'var(--surface)', color: 'var(--ink)' }}
      >{period === 'day' ? '주간' : '야간'}</button>
    </div>
  )
}
  ...
  return (
    <>
      ...
      <nav
        ...
        style={{ background: 'var(--surface)', borderRight: '1px solid var(--border-color)', fontFamily: SPOT_FONT }}
      >
        <div className="text-[10px] font-bold tracking-wider px-2 pb-2.5" style={{ color: 'var(--ink-60)' }}>
          SPOTTER
        </div>
        {NAV_LINKS.map(l => (
          <NavLink
            key={l.to} to={l.to}
            style={({ isActive }) => ({
              display: 'block', padding: '8px 10px', borderRadius: 8, textDecoration: 'none', fontSize: 12,
      ...
      <div
        ...
        style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border-color)', fontFamily: SPOT_FONT }}
      >
        <span className="text-[11px] font-bold tracking-wider" style={{ color: 'var(--ink-60)' }}>SPOTTER</span>
...
function HomeRoute()            { const { dark, period } = useUI(); return <MobileLayout><MobileHome dark={dark} period={period} /></MobileLayout> }
function MapRoute()             { const { dark, period } = useUI(); return <MobileLayout><MobileMap dark={dark} period={period} /></MobileLayout> }
function BoothRoute()           { const { dark } = useUI(); return <MobileLayout><MobileBoothDetail dark={dark} /></MobileLayout> }
function TrucksRoute()          { const { dark } = useUI(); return <MobileLayout><MobileFoodTrucks dark={dark} /></MobileLayout> }
function WaitingRegisterRoute() { const { dark } = useUI(); return <MobileLayout><MobileWaitingRegister dark={dark} /></MobileLayout> }
function WaitingStatusRoute()   { const { dark } = useUI(); return <MobileLayout><MobileWaitingStatus dark={dark} /></MobileLayout> }
function AdminFestivalRoute()   { const { dark } = useUI(); return <AdminLayout><AdminFestival dark={dark} /></AdminLayout> }
function AdminBoothsRoute()     { const { dark } = useUI(); return <AdminLayout><AdminBooths dark={dark} /></AdminLayout> }
function AdminWaitingRoute()    { const { dark } = useUI(); return <AdminLayout><AdminWaiting dark={dark} /></AdminLayout> }
